...
# 활용 도구
- lazydocker (도커 컨테이너 관리, 간편한 로그, 쉘 접속)
- akhq (카프카 브로커 설정 변경, 카프카 클러스터 정상 동작 체크, 컨슈머랙 체크)
- postman (스키마 레지스트리, 카프카 커넥트 rest 요청)
...
- 디스크 확보
  - 대용량의 데이터 혹은 변환 후 원본 대비 축약이 가능한 경우 이관/배치 처리 주기에 맞는 로그 리텐션 기간을 설정하고
  - 기간 내에 느리지만 저렴한 대용량 스토리지에 이관하거나 변환 연산을 통해 필요 데이터만 남기는 작업 수행
  - 카프카 3.0+에서 tiered storage를 활용하여 마치 L1~L4캐시 피라미드처럼 핫, 콜드 데이터를 SSD, HDD/remote(s3)로 분배하는 전략 활용
  - 
- 모니터링 / 장애 대응
  - 각 서비스들의 로그와 카프카 자체 메트릭을 ELK로 연동하여 LogStash로 읽어들이고 elasticSearch로 데이터를 인덱싱해두면
  - kibana는 elasticSearch에게 질의 요청하여 시각화를 제공하는 방식 (으로 알고 있습니다)
  - 카프카 셀프 모니터링의 경우 디스크 사용량, inode개수, 네트워크 트래픽, 지워도 되는 오래된 토픽 데이터 시각화가 가능
  - delete.topic.enable=true을 통해 긴급 상황에서 토픽을 삭제할 수 있도록 허용
- 성능 개선
  - 컨슈머 그룹 out/in에 따른 리밸런싱 과정에서 응답이 중단될 수 있으므로
  - cooperative sticky assigner 정책 설정으로 이미 붙어있는 커넥션은 유지한채
  - 새로운 할당될 파티션-컨슈머 짝만 확정까지 대기 후 연결.
  - 파티션 리더 리밸런싱
  - 특정 브로커가 비율적으로 다수의 파티션 리더역을 담당하면 핫스팟 서버가 장애/다운 가능성이 있으므로
  - 파티션 리더 리밸런싱 하이퍼파라미터를 조절
  - leader.imbalance.check.interval.seconds (체크 주기), leader.imbalance.per.broker.percentage (리더 역임 비율)
